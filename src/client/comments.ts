 ...
 */
export function initCommentsLayout (): void
{
    const boardEl = document.querySelector<HTMLElement>( '.comment-board' );
    const allWrappers = Array.from( document.querySelectorAll<HTMLElement>( '.comment-wrapper' ) );

    if ( !boardEl || allWrappers.length === 0 ) return;
    const board = boardEl;
    ...
    function layoutComments (): void
    {
        ...
        const maxCount = Math.min(
            allWrappers.length,
            Math.floor( ( area * fillRatio ) / commentArea )
        );
        ...
        {
            let x: number, y: number, tries = 0, collides: boolean;
