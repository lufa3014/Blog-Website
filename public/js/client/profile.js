...
export function initProfileForm() {
    const form = document.querySelector('#profile-data-form');
    const infoContainer = document.querySelector('.profile-info');
    const avatarContainer = document.querySelector('.profile-block.avatar-only');
    if (!form || !infoContainer || !avatarContainer)
        return;
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            const data = await response.json();
            {
                ...
                const newInfo = wrapper.querySelector('.profile-info');
                if (!newInfo)
                    throw new Error('Kein .profile-info gefunden');
            ...
            }
            {
                ...
                const wrapper = document.createElement('div');
                wrapper.innerHTML = newHtml;
                const newAvatar = wrapper.querySelector('.profile-block.avatar-only');
                if (!newAvatar)
                    throw new Error('Kein .profile-block.avatar-only-Block gefunden');
...
export function initSkills() {
    const mainEl = document.querySelector('.profile[data-user-id]');
    const userId = mainEl?.dataset.userId;
    if (!mainEl || !userId)
        return;
    const list = document.getElementById('skill-list');
    const formAdd = document.getElementById('skill-edit-form');
    const tpl = document.getElementById('skill-template');
    if (!list || !formAdd || !tpl)
        return;
    formAdd.addEventListener('submit', async (e) => {
        e.preventDefault();
        const input = formAdd.elements.namedItem('skill');
